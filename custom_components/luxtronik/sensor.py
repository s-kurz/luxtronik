...
class LuxtronikStatusSensorEntity(LuxtronikSensorEntity, SensorEntity):
    ...
    def _handle_coordinator_update(
        ...
        if not self._attr_cache[SA.EVU_DAYS] and not isinstance(self._attr_cache[SA.EVU_DAYS], list):
    ...
    def _calc_next_evu_event_minutes(self) -> int | None:
        ...
        weekday = datetime.today().weekday()
        evu_pause = 0
        if not self._attr_cache[SA.EVU_DAYS]:
            if not isinstance(self._attr_cache[SA.EVU_DAYS], list):
                self._attr_cache[SA.EVU_DAYS] = list()
                evu_pause = 6* 1440 + (24 - datetime.now().hour)*60 - datetime.now().minute
            elif weekday not in self._attr_cache[SA.EVU_DAYS]:
